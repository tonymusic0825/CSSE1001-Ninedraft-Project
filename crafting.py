...
class GridCrafterView(tk.Frame):
    ...
    def __init__(self, master, input_size):
        ...
        self._input_row, self._input_column = input_size

        self._image = None
        # Crafting Widgets
        self._input = Grid(rows=self._input_row, columns=self._input_column)
        ...
        self._output = Grid(rows=1, columns=1)
        self._output_view = ItemGridView(self, self._output.get_size())
    ...
    def render(self, key_stack_pairs, selected):
        ...
        for key, stack in key_stack_pairs:
            ...
            if key == "output":
                self._output_view.draw_cell((0,0), stack, key == selected)
            else:
                self._input_view.draw_cell(key, stack, key == selected)
    ...
    def bind_for_id(self, event, callback):
        ...
        self._input_view.bind(event, lambda e: callback(self._input_view.xy_to_grid((e.x, e.y)), e))
        self._output_view.bind(event, lambda e: callback('output', e))
        self._craft_button.bind(event, lambda e: callback('craft', e))

class FurnaceItemGridView(ItemGridView):
    """Subclass of ItemGridView, defines draw methods for rendering item oriented views"""
    def draw_image(self, grid_position):
        """Let's you create an image within the grid position (grid_position)"""
        centre = self.grid_to_xy_centre(grid_position)
        self._image = tk.PhotoImage(file="fire.ppm")
        self.create_image(centre, image = self._image)

class GridFurnaceView(tk.Frame):
    """A tkinter widget used to display furnace with a grid as input and a single cell as output"""
    def __init__(self, master, input_size):
        """Constructor
        ...
        # Furnace Widgets
        self._input = Grid(rows=self._input_row, columns=self._input_column)
        self._input_view = FurnaceItemGridView(self, self._input.get_size())
        self._input_view.pack(side=tk.LEFT)
    ...
    def render(self, key_stack_pairs, selected):
        """Renders the stacks or image at appropriate cells, as determined by 'key_stack_pairs'
                Parameters:
                    key_stack_pairs (tuple<*, Stack>):
                            (key, stack) pairs, where each stack should be drawn at the cell
                            corresponding to key
        ...
        for key, stack in key_stack_pairs:
            if key == "output":
                ...
                self._input_view.draw_image(key)
...
class CraftingWindow(tk.Toplevel):
    ...
    def __init__(self, master, title, hot_bar: Grid, inventory: Grid, crafter: GridCrafter):
        ...
        self.bind("e", lambda e: self.close_window())
    ...
    def close_window(self):
        """Closes current crafting/furnace window"""
        self.destroy()

    def _load_crafter_view(self):
        """Loads the appropriate crafter view"""
        if self._window_title == "Furnace":
            self._source_views['crafter'] = crafter_view = GridFurnaceView(self,
                                                                           self._sources['crafter'].get_input_size())
        else:
